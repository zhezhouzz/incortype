...
Definition ret_ty_of_op (op: primop): base_ty :=
  match op with
  | op_plus_one => TNat
  | op_minus_one => TNat
  | op_eq_zero => TBool
...
Definition ty_of_op (op: primop): ty := TNat ⤍ (ret_ty_of_op op).
...
| BtEffOp : forall Γ (op: primop) v1 e (T1 Tx: base_ty) T (L: aset),
    Γ ⊢t v1 ⋮v T1 ->
    (ty_of_op op) = T1 ⤍ Tx ->
    (forall (x: atom), x ∉ L -> (<[x := TBase Tx]> Γ) ⊢t e ^t^ x ⋮t T) ->
    Γ ⊢t tletprimop op v1 e ⋮t T

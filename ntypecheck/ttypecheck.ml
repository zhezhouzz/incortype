...
let type_check opctx nctx (x : term typed) (tyopt : t) : term typed =
  let rec bidirect_infer ctx (x : term typed) : term typed * Nt.t =
    match x.ty with None -> infer ctx x.x | Some ty -> (check ctx x.x ty, ty)
  and bidirect_check ctx (x : term typed) (ty : Nt.t) : term typed =
    match x.ty with
    | None -> check ctx x.x ty
    | Some ty' ->
        let ty = Nt._type_unify __FILE__ __LINE__ ty' ty in
        check ctx x.x ty
  and check ctx (x : term) (ty : Nt.t) : term typed =
    let () =
      Env.show_debug_preprocess @@ fun _ ->
      Ctx.pretty_print_judge ctx (layout_term x #: None, Nt.layout ty)
    in
    match (x, ty) with
    | Err, _ -> Err #: (Some ty)
    ...
    | Lam { lamarg; lambody }, Ty_arrow (t1, t2) ->
        ...
        let ctx' =
          NTypectx.new_to_right ctx
            (Coersion_aux._force __FILE__ __LINE__ lamarg)
        in
    ...
    | Let { if_rec; lhs; rhs; letbody }, _ ->
        ...
        let rhs =
          ...
          | true, [ self ] ->
              let ctx' =
                NTypectx.new_to_right ctx
                  (Coersion_aux._force __FILE__ __LINE__ self)
              in
        ...
        let ctx' =
          ...
          @@ List.map (Coersion_aux._force __FILE__ __LINE__) lhs
        in
        ...
        let handle_case { constructor; args; exp } =
          let constructor =
            _unify_update __FILE__ __LINE__
              ...
              constructor
          in
          let c = Coersion_aux._force __FILE__ __LINE__ constructor in
          ...
          let ctx' =
            NTypectx.new_to_rights ctx
              (List.map (Coersion_aux._force __FILE__ __LINE__) args)
          in
    ...
    | _, _ ->
        _failatwith __FILE__ __LINE__
          (spf "check: inconsistent term (%s) and type (%s)"
             (layout_term x #: None)
             (Nt.layout ty))
  and infer ctx (x : term) : term typed * Nt.t =
    let x, ty =
      match x with
      | Err ->
          failwith
      ...
      | Lam { lamarg; lambody } ->
          ...
          let ty =
            Nt.mk_arr (Coersion_aux._force __FILE__ __LINE__ lamarg).Nt.ty
              lambodyty
          in
          (Lam { lamarg; lambody }, ty)
      | AppOp (op, args) ->
          let args, argsty = List.split @@ List.map (bidirect_infer ctx) args in
          let opty = infer_op opctx op.x in
    ...
    let () =
      Env.show_debug_preprocess @@ fun _ ->
      Ctx.pretty_print_infer ctx (layout_term x #: None, Nt.layout ty)
    in
    (x #: (Some ty), ty)
  in
  match tyopt with
